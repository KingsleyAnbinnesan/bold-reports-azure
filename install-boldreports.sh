...
do
    ...
    case "$name" in
        ...
        -?|--?|--help|-[Hh]elp)
            ...
            echo "Usage: $script_name [-u|--user <USER>] [-h|--host <HOST_URL>] [-n|--nginx <true/false>] [--ssl-cert <CERT_PATH>] [--ssl-key <KEY_PATH>]"
            echo "Note: --ssl-cert and --ssl-key are required if the host URL uses HTTPS."
	...
	if [[ $1 =~ $url_regex ]]; then 
        ...
        if [[ $1 == https://* ]]; then
            ...
            if [[ -z "$ssl_cert_path" || -z "$ssl_key_path" ]]; then
                say_err "SSL certificate and key paths (--ssl-cert and --ssl-key) are required for HTTPS URLs."
                return 1
            fi
